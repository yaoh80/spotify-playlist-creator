...
def create_and_add_podcast_episodes():
    ...
    if not show_id:
        messagebox.showerror("Error", "ポッドキャストIDを入力してください。")
        return
    ...
    try:
        ...
        try:
            ...
            messagebox.showerror(
                "Error", "指定されたポッドキャストIDが見つかりませんでした。"
            )
        ...
        if playlist_title in existing_playlist_names:
            messagebox.showinfo(
                "Information",
                f"「{playlist_title}」というプレイリストは既に存在します。",
            )
            progressbar.pack_forget()
            return
        ...
        new_playlist = sp.user_playlist_create(
            user=sp.me()["id"], name=playlist_title, public=True
        )
        ...
        if not new_episode_uris:
            message_label.config(
                text="✅ エピソードが見つかりませんでした。", fg="green"
            )
            progressbar.pack_forget()
            return
        ...
        # 完了メッセージを生成し、幅に合わせて改行
        success_message = f"「{playlist_title}」プレイリストを新規作成し、{total_to_add}件のエピソードを追加しました！"
        wrapped_message = textwrap.fill(success_message, width=45)

        message_label.config(text=f"✅ {wrapped_message}", fg="green")
        progressbar.pack_forget()

    except Exception as e:
        messagebox.showerror("Error", f"予期せぬエラーが発生しました。\n詳細: {e}")
        progressbar.pack_forget()
...
# --- GUI要素の配置 ---
tk.Label(
    window, text="ポッドキャストから新規プレイリストを作成", font=("游ゴシック", 14)
).pack(pady=(10, 5))
...
ttk.Button(window, text="作成開始", command=create_and_add_podcast_episodes).pack(
    pady=5
)
